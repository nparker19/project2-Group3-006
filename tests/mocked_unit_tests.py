...
class UpdateDBTests(unittest.TestCase):
    ...
    def test_update_db_ids_for_user(self):
        with patch("models.User.query") as mock_query:
            with patch("models.db.session.add", self.mock_add_to_db):
                with patch("models.db.session.delete", self.mock_delete_from_db):
                    with patch("models.db.session.commit", self.mock_db_commit):
